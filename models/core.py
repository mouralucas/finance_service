...
class CategoryExpenseTypeUserModel(SQLModel):
    # TODO: creating cache for this info, if is available get from cache, update cache when user update this info
    """
    Created by: Lucas Penha de Moura - 29/10/2024
...
class IndexerTypeModel(SQLModel):
    ...
    Created by: Lucas Penha de Moura - 12/08/2024
        ...
        It's a small table that, at first, only stores 3 register (fixed, floating and hybrid)
    """
...
class IndexerSeriesModel(SQLModel):
    ...
    value: Mapped[Decimal] = mapped_column('value', Numeric(precision=15, scale=5), nullable=True)
