...
class InvestmentModel(SQLModel):
    ...
    custodiam: Mapped["BankModel"] = relationship(  # noqa: F821
        foreign_keys=[custodian_id], lazy="subquery"
    )
    ...
    type: Mapped[InvestmentTypeModel] = relationship(
    ...
    quantity: Mapped[Decimal] = mapped_column(
        "quantity", Numeric(precision=9, scale=3), nullable=True
    )
    price: Mapped[Decimal] = mapped_column(
        "price", Numeric(precision=15, scale=5), nullable=True
    )
    amount: Mapped[Decimal] = mapped_column(
        "amount", Numeric(precision=15, scale=5)
    )  # quantity*price
    ...
    settlement_date: Mapped[date] = mapped_column(
        "liquidation_date", nullable=True
    )  # TODO: change name in database
    settlement_amount: Mapped[Decimal] = mapped_column(
        "liquidation_amount", Numeric(precision=15, scale=5), nullable=True
    )  # TODO: change name in database

    country_id: Mapped[str] = mapped_column(ForeignKey("country.id"))
    country: Mapped["CountryModel"] = relationship(  # noqa: F821
    ...
    objective: Mapped["InvestmentObjectiveModel"] = relationship(
        "InvestmentObjectiveModel", foreign_keys=[objective_id], lazy="subquery"
    )
