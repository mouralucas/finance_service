...
class InvestmentBrazilianFundsModel(InvestmentBaseModel):
    ...
    Created by: Lucas Penha de Moura - 02/04/2025
        ...
        It inherits from the InvestmentBase and adds specific
            fields for funds investments.
    """
    ...
    investment_quotation_date: Mapped[date] = mapped_column(
        "investment_quotation_date", doc="The day the investment was quoted"
    )
    investment_settlement_date: Mapped[date] = mapped_column(
        "liquidation_settlement_date",
        doc="The date the investment is liquidated in the fund",
    )
    redemption_quotation_date: Mapped[date] = mapped_column(
        "redemption_quotation_date", nullable=True
    )
    redemption_settlement_date: Mapped[date] = mapped_column(
        "redemption_settlement_date", nullable=True
    )
    settlement_amount: Mapped[Decimal] = mapped_column(
        "settlement_amount", Numeric(precision=18, scale=8), default=0
    )
...
class InvestmentBrazilianFundsStatementModel(InvestmentStatementBaseModel):
    ...
    Created by: Lucas Penha de Moura - 02/04/2025
        ...
        It inherits from the InvestmentStatementBaseModel and adds specific fields
    ...
    contribution: Mapped[Decimal] = mapped_column(
        ...
        default=0.0,
        doc="The amount of money contributed to the fund in the period",
    )
    price: Mapped[Decimal] = mapped_column(
        "price",
        Numeric(precision=18, scale=8),
        doc="The price of the fund in the reference day",
    )
    penalty: Mapped[Decimal] = mapped_column(
        "penalty",
        Numeric(precision=18, scale=8),
        default=0.0,
        doc="The penalty applied to the investment in the period",
    )
