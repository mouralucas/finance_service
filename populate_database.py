...
async def populate():
    async with sessionmanager.session() as session:
        ...
        # Credit Card data
        await BaseDataManager(session).add_or_ignore_all(CreditCardModel, get_credit_card_mocked())
        await BaseDataManager(session).add_or_ignore_all(CreditCardModel, get_cancelled_credit_card_mocked())

        # Investment data
        await BaseDataManager(session).add_or_ignore_all(InvestmentTypeModel, get_investment_type_mocked())
        await BaseDataManager(session).add_or_ignore_all(InvestmentModel, get_investment_mocked())
