...
class FundsBrSchema(BaseModel):
    ...
    id: uuid.UUID = Field(
        ...,
        serialization_alias="fundId",
        description="The unique identification for the fund",
    )
    name: str = Field(..., description="The name of the fund")
    fund_cnpj: str = Field(
        ..., min_length=14, max_length=18, description="The cnpj of the fund"
    )
    administrator: str = Field(..., description="The administrator of the fund")
    administrator_cnpj: str = Field(
        ...,
        min_length=14,
        max_length=18,
        description="The cnpj of the administrator of the fund",
    )
    ...
    minimum_balance: float = Field(
        ..., description="The minimum balance to stay investing in the fund"
    )
    minimum_investment: float = Field(
        ..., description="The minimum amount of each investment"
    )
    ...
    investment_quotation: str = Field(
        ..., description="The number of days until the quotation after the investment"
    )
    redemption_quotation: str = Field(
        ..., description="The number of days until the quotation after the redemption"
    )
    redemption_settlement: str = Field(
        ...,
        description="The number of days until the redemption \
            is settled to the investor",
    )

    # TODO: maybe change the TaxFeeQuote for one schema not in request folder
    fees: list[TaxFeeQuotationSchema] | None = Field(
        None, description="The fee details of the fund fees"
    )
...
class IndexerSeries(BaseModel):
    ...
    date: datetime.date | None = Field(
        None, description="The reference date of the value"
    )
    ...
    periodocity_name: str | None = Field(
        None, description="The name of the periodicity"
    )
