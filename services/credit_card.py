...
class CreditCardService(BaseService):
    ...
    async def create_credit_card(
        ...
        new_credit_card = await CreditCardManager(
            session=self.session
        ).create_credit_card(new_credit_card)
    ...
    async def cancel_credit_card(
        self, credit_card: CancelCreditCardRequest
    ) -> CancelCreditCardResponse:
        current_credit_card = await CreditCardManager(
            session=self.session
        ).get_credit_card_by_id(card_id=credit_card.id)
        if not current_credit_card or not current_credit_card.active:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Credit card not found or not valid",
            )
        ...
        updated_credit_card = await CreditCardManager(
            session=self.session
        ).update_credit_card(current_credit_card, clean_fields)
    ...
    async def create_transaction(
        ...
        if not credit_card or not credit_card.active:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Credit card not valid"
            )
        ...
        for i in transaction.installments:
            new_bill_entry = CreditCardTransactionModel(
            ...
            new_bill_entry.is_installment = (
                True if len(transaction.installments) > 1 else False
            )

            # If it's not an international transaction,
            #   currency and amount are the same as the indicated before
    ...
    async def get_transactions(
        ...
        transactions = await CreditCardManager(
            session=self.session
        ...
        response = GetCreditCardTransactionResponse(
            ...
            transactions=(
                [
                    CreditCardTransactionSchema(**transaction)
    ...
    async def get_credit_card_bill_evolution(
        ...
        bill_consolidated = await self.credit_card_manager.get_bill_history_aggregated(
            owner_id=self.user["user_id"],
            start_period=params.start_period,
            end_period=params.end_period,
        )
        average = (
            sum(item["total_amount"] for item in bill_consolidated)
            / len(bill_consolidated)
            if bill_consolidated
            else 0
        )
        ...
        response = GetCreditCardBillConsolidatedResponse(
            bill=(
                [
                    CreditCardBillSchema.model_validate(bill)
                    for bill in bill_consolidated
                ]
                if bill_consolidated
                else []
            ),
    ...
    async def get_credit_card_bill_history(
        self, params: GetCreditCardBillRequest
    ) -> GetCreditCardBillHistoryResponse:
        bill_by_card = await self.credit_card_manager.get_bill_history_by_card(
            owner_id=self.user["user_id"],
            start_period=params.start_period,
            end_period=params.end_period,
        )
        ...
        for i in bill_by_card:
            ...
            # The totals are segregated by currency symbol, so we need to check if
            #   the currency symbol is already in the list
            # If the currency symbol is already in the list, we just need to update
            #   the total amount
            for dic in bill_periods[period]["total_amount"]:
                if dic["currency_symbol"] == currency_symbol:
                    ...
                    bill_periods[period]["total_amount"].append(
                        {"currency_symbol": currency_symbol, "total": total_amount}
                    )
            else:
                bill_periods[period]["total_amount"].append(
                    {"currency_symbol": currency_symbol, "total": total_amount}
        ...
        response = GetCreditCardBillHistoryResponse(
            credit_card_bill_history=[
                CreditCardBillHistorySchema.model_validate(i) for i in bill_history
            ],
        )
    ...
    async def get_installments_due_date(
        ...
        installments_due_dates = get_installments_due_dates(
            ...
            close_day=credit_card.close_day,
