...
async def create_investment_type(create_test_session) -> list[InvestmentTypeSchema]:
    data_ = await BaseDataManager(create_test_session).add_or_ignore_all(InvestmentTypeModel, get_investment_type_mocked())
...
@pytest_asyncio.fixture
async def create_investment(create_test_session, create_open_account, create_investment_type, create_currency,
                            create_index_type, create_index, create_liquidity, create_country) -> list[InvestmentSchema]:

    data_ = await BaseDataManager(create_test_session).add_or_ignore_all(InvestmentModel, get_investment_mocked())
...
@pytest_asyncio.fixture
async def create_investment_statement(create_test_session, create_investment) -> list[InvestmentStatementSchema]:
    data_ = await BaseDataManager(create_test_session).add_or_ignore_all(InvestmentStatementModel, get_investment_statement_mock())
