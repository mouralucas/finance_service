...
async def test_create_transaction_no_installment(client, create_valid_credit_card, create_category, create_currency):
    ...
    tot_installments = 1
    installments_dates = get_installments_due_dates(transaction_date=datetime.datetime.strptime(transaction_date, '%Y-%m-%d'),
                               close_day=close_day, due_day=due_day, tot_installments=tot_installments)
