...
@pytest.mark.asyncio
async def test_create_funds_br_investment(
    client,
    ...
    country = countries[0]
    objective = None
    investment_quotation_date = get_randon_date(
        start_date=get_timestamp_aware() - relativedelta(months=3),
        end_date=get_timestamp_aware(),
    )
    investment_settlement_date = investment_quotation_date + relativedelta(days=2)
    redemption_quotation_range = "D+2 (dias úteis)"
    redemption_quotation_date = get_randon_date(
        start_date=get_timestamp_aware(),
        end_date=get_timestamp_aware() + relativedelta(months=5),
    )
    ...
    payload = {
        ...
        "redemptionSettlementDate": str(
            redemption_settlement_date.strftime("%Y-%m-%d")
        ),
    ...
    assert (
        payload is not None
    )  # Placeholder assertion, replace with actual API call when implemented
    assert True
