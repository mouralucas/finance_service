...
@pytest.mark.asyncio
async def test_create_brazilian_fund_investment(
    client,
    ...
    ] == investment_quotation_date.strftime("%Y-%m-%d")
...
@pytest.mark.asyncio
async def test_get_brazilian_fund_investment_statement(
    client, create_brazilian_fund_investment_statement
):
    statements = create_brazilian_fund_investment_statement
    fund_id = statements[0].fund_id

    payload = {"fund_id": str(fund_id)}
